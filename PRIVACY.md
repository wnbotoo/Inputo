...
Future file tools are designed to be grant-scoped. Web code must not send arbitrary local paths to native code. A grant should be created through a native picker or save panel before file read or write tools can operate.

## Diagnostics

Diagnostic output should avoid secrets, prompts, generated text, provider URLs with credentials, local paths, screenshots, and stack traces that reveal sensitive user data. Public issues and pull requests should use redacted examples.
